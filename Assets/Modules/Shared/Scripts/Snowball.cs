...
namespace Niantic.ARVoyage
{
    ...
    {
        private const float snowballScale = 1f;
        ...
        // lcw
        public int pitchNum = 0;
        float tossAngleY = 0;
        float gravForce = 0.2f;
        Renderer rdr;

        public float curveUpForce = 0.5f;
        public float curveRightForce = 0.5f;
        ...
        private float bounceDamper = .03f;       // Multiplier to dampen bounce force.
        ...
        // lcw
        public Vector3 MakerUp;
        public Vector3 MakerRight;
        ...
        {
            // lcw
            SnowballMaker sbm = this.GetComponentInParent<SnowballMaker>();
            pitchNum = sbm.currPitchNum;
            tossForce = sbm.currBallForce;
            tossAngleY = sbm.currBallAngleY;
            curveUpForce = sbm.currCurveUpForce;
            curveRightForce = sbm.currCurveRightForce;
            

            rdr = snowballModel.GetComponent<MeshRenderer>();
        }
        ...
        // lcw - 구질 종류에 따라 AddForce 방향 바꿈
        private void FixedUpdate()
        {
            if(!IsHeld)
            {
                // Vector3 grav = -1 * this.transform.up * gravForce;
                // snowballRigidbody.AddForce(grav );    // Up curve


                if(isDestroying){
                    tmpColor = rdr.material.color;
                    tmpColor.a = tmpColor.a - 0.05f;
                    rdr.material.color = tmpColor;
                    Debug.LogWarning("#####################");
                    Debug.LogWarning(rdr.material.color.a);
                ...
                if(impactCount == 0)
                {
                    snowballRigidbody.AddForce( MakerUp * curveUpForce);
                    snowballRigidbody.AddForce( MakerRight * curveRightForce);


                    // if(pitchNum==0)     
                    // {
                    //     // snowballRigidbody.AddForce(this.transform.up*1.0f );    // Up curve
                    //     // snowballRigidbody.AddForce(MakerUp*0.98f );    // Up curve
                    // }else if(pitchNum==1)      
                    // {
                    //     snowballRigidbody.AddForce(-1 * MakerUp*0.5f );    // Down curve
                    // }else if(pitchNum==2)      
                    // {
                    //     snowballRigidbody.AddForce(-1 * MakerRight *0.5f );    // Left curve
                    // }else if(pitchNum==3)      
                    // {
                    //     snowballRigidbody.AddForce(this.transform.right*0.5f );    // Right curve
                    // }else if(pitchNum==4)      
                    // {
                    //     snowballRigidbody.AddForce(-1 * MakerUp*0.5f );    // Down curve
                    //     snowballRigidbody.AddForce(MakerRight*0.5f );    // Right curve
                    // }
                }else{
                    // 중력
                    snowballRigidbody.AddForce(-1 * MakerUp*0.98f );    // Down curve
                }
        ...
        public void TossLocallySpawnedSnowball(float tossAngle, float tossAngleY)
        {

            Vector3 force = transform.forward * tossForce;
            Vector3 torque = transform.right * Random.Range(1, 3);

            TossSnowball(tossAngle, force, torque, tossAngleY);
        ...
        private void TossSnowball(float tossAngle, Vector3 force, Vector3 torque, float tossAngleY=0)
        {
            ...
            // snowballRigidbody.AddTorque(this.transform.up * Random.Range(400, 500));
        ...
        public void HandleCollision(Collision collision, bool destroy = true)
        {
            ...
            // if (!(collision.gameObject.layer == AR_MESH_LAYER || collision.gameObject.layer == AR_PLANES_LAYER)) // lcw - 환경 말고 다른곳에 닿았을 떄?
            // {
            ...
            if (impactCount < maxImpacts)           // lcw - 환경에 첫번째로 닿았을 떄?
            {

...
namespace Niantic.ARVoyage
{
    ...
    {
        public const float defaultTossAngle = 0f;      // lcw
        ...
        public GameObject ChangeBallPanel;


        public int currPitchNum = 0;
        public int currBallForce = 5;
        public int currBallAngle = 0;
        public int currBallAngleY = 0;
        public float currCurveUpForce = 0;
        public float currCurveRightForce = 0;
        ...
        private void Update()
        {
            ...
            {
                Debug.LogWarning("Pressed Long# : OpenChangeBallPopup()");
                OpenChangeBallPopup();
                isClick = false;
            }
        ...
        public void TossSnowball()
        {
            ...
            // lcw
            // heldSnowball.MakerUp = this.transform.up;
            // heldSnowball.MakerRight = this.transform.right;
            heldSnowball.MakerUp = Camera.main.transform.up;
            heldSnowball.MakerRight = Camera.main.transform.right;
            ...
            // Toss the snowball
            heldSnowball.TossLocallySpawnedSnowball(tossAngle, tossAngleY);
            Debug.LogWarning("tossAngleY : " + tossAngleY);
        ...
        public void ButtonUp()
        {
            if(clickTime >= minClickTime)
            {
                ...
                if(isClick){
                    Debug.LogWarning("Pressed Once! : TossSnowBall()");
                    TossSnowball();
                }
            }
            isClick = false;
        ...
        public void OpenChangeBallPopup()
        {
            ChangeBallPanel.SetActive(true);
        }


        public void SetBallForce(int i){
            currBallForce = i;
            Snowball sb = this.GetComponentInChildren<Snowball>();
            sb.tossForce = currBallForce;
        }
        public void SetBallAngle(int i){
            currBallAngle = i;
            tossAngle = currBallAngle;
        }
        public void SetBallAngleY(int i){
            currBallAngleY = i;
            tossAngleY = currBallAngleY;
        }

        public void SetCurveUpForce(int i){
            currCurveUpForce = 0.001f * i;

            // currCurveUpForce = (float)i/10f;
            Snowball sb = this.GetComponentInChildren<Snowball>();
            sb.curveUpForce = currCurveUpForce;

        }
        public void SetCurveRightForce(int i){
            // currCurveRightForce = (float)i/10f;
            currCurveRightForce = 0.001f * i;
            Snowball sb = this.GetComponentInChildren<Snowball>();
            sb.curveRightForce = currCurveRightForce;
        }
